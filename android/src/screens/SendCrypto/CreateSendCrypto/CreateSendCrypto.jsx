...
import { TatumSDK, Network, Currency } from '@tatumio/tatum'; // Import Tatum SDK
...
const CreateSendCrypto = ({navigation, route}) => {
  ...
  const [serviceFee, setServiceFee] = useState(0); // Add state for service fee
  ...
  const calculateServiceFee = (amount) => {
    return parseFloat(amount || 0) * 0.05;
  };

  // Update service fee when amount changes
  useEffect(() => {
    setServiceFee(calculateServiceFee(amount));
  }, [amount]);
  ...
  const handleProceed = () => {
    ...
    // Include service fee in the total
    const totalAmount = parseFloat(amount) + serviceFee;

    navigation.navigate(CONFIRM_SEND_CRYPTO, {
      data: {
        formData,
        amount,
        serviceFee, // Pass service fee to confirmation screen
        totalAmount, // Pass total amount including fee
        networkFee,
        setFormData,
        initialState,
        setAmount,
            ...
            <View style={styles.container}>
              ...
              {amount > 0 && (
                <View style={styles.feeContainer}>
                  <Text style={styles.feeLabel}>{trans('Service Fee (5%)')}: </Text>
                  <Text style={styles.feeAmount}>
                    {serviceFee.toFixed(decimal_format_amount_crypto)} {formData?.code}
                  </Text>
                </View>
              )}
              {/* Display total amount including fee */}
              {amount > 0 && (
                <View style={[styles.feeContainer, styles.mb_16]}>
                  <Text style={styles.totalLabel}>{trans('Total Amount')}: </Text>
                  <Text style={styles.totalAmount}>
                    {(parseFloat(amount) + serviceFee).toFixed(decimal_format_amount_crypto)} {formData?.code}
                  </Text>
                </View>
              )}
              <View style={styles.mb_16}>
