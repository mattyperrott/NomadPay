...
import { SUCCESS_SEND_CRYPTO_TOKEN } from '../../../navigation/routeName/routeName';
...
const ConfirmSendCryptoToken = ({
  ...
  const {
    amount, networkFee, setFormData, initialState, setAmount,
    formData: { code, recipientAddress, network, typeID, type } = {},
  } = data;
  ...
  const handleConfirm = async () => {
    ...
    if (type == 'crypto_token') {
      URL = `${config.BASE_URL_VERSION}/token/send/tatumio/confirm`;
    }
    const obj = {
    ...
    if (result) {
      ...
      if (status?.code === 200) {
        navigation.navigate(SUCCESS_SEND_CRYPTO_TOKEN, {
          data: {
            floatAmount,
            code,
            recipientAddress,
        ...
        <ScrollView
          ...
          <View style={styles.container}>
            <TransactionStep
              ...
              presentStep={2}
              totalStep={2}
              description={trans('Take a look before you send. Once the token sent to this address, its never be undone.')}
            />
            ...
            <View>
              <Text style={[styles.info, styles.contentWidth]}>{trans('Network Fee will be deduct from your native {{x}} wallet.', { x: network })}</Text>
            </View>
            <View>
              <DetailsCard
                style={{
...
export default ConfirmSendCryptoToken;
